 ...
 */
public class SensorTestTest extends ActivityInstrumentationTestCase2<SensorTest> {
    public static final String TAG = "SensorTestTest";
    ...
    public void testUsingMockLocation() {
        ...
        assertEquals(mSensorTest.mCurrentLocation.getLatitude(),
                testLocation.getLatitude(), 0.000001f);
        assertEquals(mSensorTest.mCurrentLocation.getLongitude(),
                testLocation.getLongitude(), 0.000001f);
        assertEquals(mSensorTest.mLatitudeTextView.getText().toString(),
                String.valueOf(testLocation.getLatitude()));
        assertEquals(mSensorTest.mLongitudeTextView.getText().toString(),
    ...
    private void ensureGoogleApiClientConnection() {
        if (!mSensorTest.mGoogleApiClient.isConnected()) {
            mSensorTest.mGoogleApiClient.blockingConnect();
        }
    }
    ...
    private void confirmPreconditions() {
        assertNotNull("mSensorTest is null", mSensorTest);
        assertTrue("GoogleApiClient is not connected", mSensorTest.mGoogleApiClient.isConnected());
    }
    ...
    private void clickStartUpdatesButton() {
        mSensorTest.runOnUiThread(new Runnable() {
            public void run() {
                mSensorTest.mStartUpdatesButton.performClick();
            }
        });
    }
    ...
    private void setMockLocation(final Location mockLocation) {
        ...
        LocationServices.FusedLocationApi.setMockMode(mSensorTest.mGoogleApiClient, true)
